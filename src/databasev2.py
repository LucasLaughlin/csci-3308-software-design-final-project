...
myDatabase = '/var/www/sensorlog.db'
#devive file
...
def storage(temp):
    ...
    db_cursor.execute("INSERT INTO temps (timestamp, temp) values(datetime('now'), (?))", (temp,))
...
def display():
    ...
    for row in db_cursor.execute("SELECT timestamp, temp FROM temps"):
